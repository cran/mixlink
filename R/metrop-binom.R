...
rwmetrop.mixlink.binomial <- function(y, X, m, R, burn = 0, thin = 1,
	...
	hyper = NULL, report.period = R+1, use.laplace.approx = TRUE, proposal = NULL,
	param.grp = NULL, fixed.kappa = FALSE)
{
    ...
    y = y, m = m, X = X, invlink = invlink, report.period = report.period)
...
#' @name S3 methods for rwmetrop output objects
DIC.rwmetrop.mixlink.binomial <- function (object, ...)
{
  y <- object$y
  m <- object$m
  X <- object$X
	...
	for (r in 1:R.keep) {
      ...
      logger("Computing DIC for rep %d\n", r)
		...
		mean.hat <- object$invlink(X %*% Beta)
	...
	mean.bar <- object$invlink(X %*% Beta.bar)
...
#' @name S3 methods for rwmetrop output objects
residuals.rwmetrop.mixlink.binomial <- function(object, ...)
{
  y <- object$y
  X <- object$X
  m <- object$m
